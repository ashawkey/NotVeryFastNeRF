...
class ImageViewWidget(gl.GLViewWidget):
    ...
    def __init__(self, **kwargs):
        ...
        self.window_size = (800, 800) # (512, 512)
        self.focal = 3804 * (self.window_size[0] / 4032)
        ...
        self.inds = torch.from_numpy(np.load('../fastnerf/output/silica_hr_inds_768_0.npy')).int().cuda()
        print(f'load inds: {self.inds.shape} {self.inds.dtype}')
        self.uvws = torch.from_numpy(np.load('../fastnerf/output/silica_hr_uvws_768_0.npy')).float().cuda()
        print(f'load uvws: {self.uvws.shape} {self.uvws.dtype}')
        self.beta = torch.from_numpy(np.load('../fastnerf/output/silica_hr_beta_128_cart.npy')).float().cuda()
